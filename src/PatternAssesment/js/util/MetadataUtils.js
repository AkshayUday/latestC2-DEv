...
export default {
        ...
        if(typeof(keywordsItr) === 'string'){
           ...
           if(keywordsItr.length === undefined){
              ...
              for(let key in keywordsItr){
                ...
                /*let my_object_keys = Object.keys(keywordsItr);
                for(let i=0, l=my_object_keys.length; i<l; i++) {
                  itemArr.push({'id': my_object_keys[i], 'name': keywordsItr[i]});
                }*/
            }  
        }
  ...
  copy(src,target){
      if(src && target){
        ...
        for(let i=0, l=src_keys.length; i<l; i++) {
          itemArr.push({'id': my_object_keys[i], 'name': keywordsItr[i]});
          if(src[i] && src[i]!=''){
                  target[i] = src[i];
          }
