import * as actions from '../../../PatternAssesment/js/action/MetadataAction'
import expect from 'expect' //used any testing library
import metadataConstants from '../../../PatternAssesment/js/constants/MVMConstants';
import marklogicMetadataAPI from '../../../PatternAssesment/js/api/MetadataAPI';
import store from '../../../PatternAssesment/js/store';
import Promise from 'bluebird';
import MetaDataService from '../../../common/util/metadataService';
import * as types from '../../../PatternAssesment/js/constants/MVMConstants';
...
/***********For saveMetaData Function ********/
 ...
 describe('stub api and returning save sucess should work', () => {
  ...
  it('should return a function in Assessment Metadata', () => {
     expect(actions.saveMetaData()).toBeA('function');
   })

  it('dispatch a Save AssessmentMetaData action for SET_ON_LOAD_VALUE', () => {
  ...
  it('dispatch a Save AssessmentMetaData action for SET_UUID', () => {
    actions.saveMetaData({"name":"","desc":"aaa"})(store.dispatch, store.getState);
    setTimeout(function(){
...
describe('stub api and returning save update sucess should work', () => {
  ...
  it('dispatch a Save Update AssessmentMetaData action for SET_ON_LOAD_VALUE', () => {
  ...
  it('dispatch a Save Update AssessmentMetaData action for SET_UUID', () => {
  ...
  it('dispatch should not happen a Save AssessmentMetaData action', () => {
    actions.saveMetaData({"name":"","desc":"aaa"})(store.dispatch,store.getState);
...
describe('stub api and returning update save error should work', () => {
  var dispacthSpy1;
  var getStateSpy1;
  let data;
  before(function () {
  ...
  it('dispatch should not happen a Save AssessmentMetaData action', () => {
    actions.saveMetaData({"name":"","desc":"aaa","uuid":"12345"})(store.dispatch,store.getState);
    expect(dispacthSpy1.called).toEqual(false);
  });
  after(function () {
    dispacthSpy1.restore();
...
/* describe('async actions', () => {
  ...
  before(function () {
     ...
     server.respondWith("POST", "https://staging.data.pearson.com/thing?db=qa2", [
    200,
    {"Content-Type": "application/json"},
  ...
  it('Save Question Metadata should work fine when mocking service', () => {
    actions.saveMetaData(libConfig)(store.dispatch, store.getState);
    server.respond();
...
describe('stub api and returning sucess should work for Audience Taxonomydata', () => {
  ...
  it('dispatch a AssessmentMetaData Audience Taxonomydata action', () => {
    actions.fetchMetaDataTaxonomy(metadata)(store.dispatch);
    setTimeout(function(){
      let dispatchArg = dispacthSpy.args[0][0];
      expect(dispacthSpy.called).toEqual(true);
  ...
  it('dispatch a AssessmentMetaData DifficultyLevel Taxonomydata action', () => {
    actions.fetchMetaDataTaxonomy(metadata)(store.dispatch);
    setTimeout(function(){
      let dispatchArg = dispacthSpy.args[0][0];
      expect(dispacthSpy.called).toEqual(true);
  ...
  it('dispatch a AssessmentMetaData Domain Taxonomydata action', () => {
    actions.fetchMetaDataTaxonomy(metadata)(store.dispatch);
    setTimeout(function(){
      let dispatchArg = dispacthSpy.args[0][0];
      expect(dispacthSpy.called).toEqual(true);
 ...
 describe('stub api and returning error should work', () => {
  ...
  it('dispatch a AssessmentMetaData action', () => {
...
/* describe('async actions', () => {
  ...
  before(function () {
     ...
     server = sinon.fakeServer.create();
     server.respondWith("GET", "http://localhost:3000/AssessmentMetaData", [
    200,
    {"Content-Type": "application/json"},
    JSON.stringify({"name":"assessment1"})
    ]);
...
describe('stub api and returning sucess should work', () => {
 ...
 it('dispatch original AssessmentMetaData action', () => {
    actions.fetchMetaData({"uuid":"6e143c65-6abd-4af4-80fd-e1a657c8132f"})(store.dispatch,store.getState);
    setTimeout(function(){
      let dispatchArg = dispacthSpy.args[0][0];
      expect(dispacthSpy.called).toEqual(true);
...
describe('stub api and returning error should work', () => {
 ...
 /*describe('async actions', () => {
  ...
  var server;
  var metadata = {};
  metadata.uuid = "6e143c65-6abd-4af4-80fd-e1a657c8132f";
  metadata.libConfig = {'locale': 'en_US',
...
'AQIC5wM2LY4SfczJwDEKiLveBuH9DtGGXLkfvRlveimpxgQ.*AAJTSQACMDIAAlNLABM0NjAwMDQ4NTYxOTkyNTk3NTU1AAJTMQACMDE.*'
                               },
                   'database'       : '?db=qa2',
                   'server'         : 'https://staging.data.pearson.com',
       'taxonomyserver' : 'https://staging.schema.pearson.com',
  ...
  before(function () {
     ...
     dispacthSpy = sinon.spy(store,"dispatch");
     server = sinon.fakeServer.create();
     server.respondWith("GET", "https://staging.data.pearson.com/work/6e143c65-6abd-4af4-80fd-e1a657c8132f?db=qa6", [
    200,
    {"Content-Type": "application/json"},
    JSON.stringify({"uuid":"6e143c65-6abd-4af4-80fd-e1a657c8132f","name":  "assessment1"})
    ]);

  });

  it('fetch Question Metadata should work fine when mocking service', () => {
    //actions.fetchMetaData(metadata)(store.dispatch,store.getState);
    server.respond();
    //console.log("server respond called");
    setTimeout(function(){
      let dispatchArg = dispacthSpy.args[0][0];
      ...
      expect(dispatchArg.QMD_Data.name).toEqual("assessment1");
    },100)
  });
