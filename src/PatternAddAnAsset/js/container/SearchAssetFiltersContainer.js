...
const getCurrentValues = (dataArray) => {
...
const mapDispatchToProps = (dispatch) => {
  return {
    tabHandleSelect: function (index, last) {
      ...
      let filterData = getCurrentValues(state.userPreferenceReducer);
      if(index===4){
         dispatch({
      ...
      if(index!==4){
        if(document.querySelector('#searchAutoSuggest input')){
          ...
          dispatch(getSearchProductItems(searchValue,DEFAULT_PAGE_NO,filterData.displayValueCount,index,filterData.sortIndex));
        }
      }else{
